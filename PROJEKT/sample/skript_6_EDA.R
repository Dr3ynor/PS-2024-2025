...
# Rozsáhlý skript lze rozdělit do tzv. SEKCÍ.
# Část kódu, která tvoří sekci, označíme a zvolíme Code \ Insert Section 
# nebo klávesovou zkratku CTRL+SHIFT+R. V dialogovém okně následně zadáme název sekce,
# například „1. Příprava prostředí“ a potvrdíme tlačítkem OK.
# Sekce se rovněž vytváří automaticky řádkem komentáře (začíná #), který obsahuje 
# alespoň čtyři koncové pomlčky (-), rovnítka (=) nebo hashtagy (#).

# Pro sbalení/rozbalení sekcí použijte Edit \ Folding.
# Užitečné klávesové zkratky: ALT + O (Collapse All), ALT + SHIFT + O (Expand All)
...
setwd("C:/Users/elisk/Documents/School/M_1/PS/cv07")
...
data = read.csv2(file = "C:/Users/elisk/Documents/School/M_1/PS/cv07/aku_st.csv")
...
hist(dataS$kap5)
hist(dataS$kap5, breaks=20) # Co dělají různé hodnoty parametru breaks s grafem?
...
ggplot(dataS,
  ...
  theme(axis.text = element_text(color = "black", size = 13),
        axis.title = element_text(size = 13),
        plot.title = element_text(hjust = 0.5)) + # zarovnání názvu grafu na střed
  facet_wrap(~vyrobce,nrow = 2) # "facety" umožňují přípravu sad grafů stejného typu v závislosti na kategoriální proměnné
...
pom = ggarrange(hist,qq,
                nrow = 1) #widths = c(1,2) změní poměr dolních grafů
ggarrange(box,pom,
          nrow = 2,
          heights = c(2,4))
